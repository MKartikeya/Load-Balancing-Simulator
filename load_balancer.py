...
available_client_ips = ["10.0.0.1", "10.0.1.3"]
# List of available server IP addresses
available_server_ips = ["10.0.0.1", "10.0.0.2"]
...
def handle_client(client_socket,data):
    ...
    try:
        ...
        if client_ip in available_client_ips:
            ...
            client_socket.sendall("1".encode('utf-8'))  # Acknowledge successful connection
            print("Client {} connected with IP: {}".format(client_name, client_ip))
        else:
            client_socket.sendall("0".encode('utf-8'))  # Reject connection
...
def handle_server(server_socket,data):
    ...
    try:
        ...
        server_socket.sendall("1".encode('utf-8'))  # Acknowledge successful connection
...
def start_load_balancer():
    ...
    server_socket.bind(('10.0.0.2', 20001))  # Listen on load balancer IP and port
